...
<div class="container-fluid">
	<div class="block-header">
	   <span style="text-shadow: 2px 2px 2px #aaa;">MEDIUM &amp; REAGENT PREPARATION</span>
						 ...
						 <tbody>
							...
							<tr>
								...
								<c:choose>
									...
									<input type="hidden" id="r_is_weight" value="${info.isWeight}">
...
function add(el) {
	$("#id").val("");
	$("#reagentId").select2("val", "");
	//$("#reagentId").val("").trigger('change.select2');
	$("#batchNo").val("");
	$("#batchSize").val("");
    $("#prepDate").val("");
	$("#expDate").val("");
...
function edit(el) {
	...
	var prepDate    = $(el).closest("tr").find("#r_prep_date").val();
	var expDate    = $(el).closest("tr").find("#r_exp_date").val();
	
		
	$("#id").val(id);
	$('#reagentId').attr("disabled", true);
	$("#reagentId").select2("val", reagentId);
	 
	$("#batchNo").val(batchNo);
	$("#batchSize").val(batchSize);
    $("#prepDate").val(convertMmDate(prepDate));
...
function getBatchInfo(id){
	...
	if(pkId==''){
	var idNo=$("#"+id+"").val();
	$.get( "${pageContext.request.contextPath}/reagent/generateBatchInfo/"+idNo, 
	function( data ) {
		$("#batchSeqNo").val(data.batchSeqNo);
		$("#batchNo").val(data.labBatchNo);
	});
...
$("#infoForm").submit(function(event){
    ...
    if($(".alert-code").hasClass('hidden')){
    	$.ajax({	
	    	url : "${pageContext.request.contextPath}/logbook_micro/saveMediumReagentInfo",
	        type: 'POST',
	        data: formData,
	        async: false,
