...
<div class="container-fluid">
	<div class="block-header">
	   <span style="text-shadow: 2px 2px 2px #aaa;">MULTI PARAMETER LOGBOOK INFO</span>
		...
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="card">
				<div class="body table-responsive table-bordered">
					...
					<c:if test="${not empty multiParamInfos}">
					...
					<table class="table table-bordered table-striped table-hover js-basic-example dataTable" id="typeList">
						<thead>
							<tr>
								...
								<th class="align-center" style="width: 100px;">ITEM NAME</th>
						...
						<tbody>
							...
							<tr>
								...
								<td class="align-center">${info.itemName}</td>
								<td class="align-center">
									<fmt:formatDate pattern="dd-MMM-yyyy" value="${info.operateDate}" var="dateVal" />
	                              	<c:out value="${dateVal}"/>
								</td>
								...
								<td class="align-center">
									...
									<input type="hidden" id="r_item_id" value="${info.itemId}">
									<input type="hidden" id="r_operate_date" value="${info.operateDate}">
									<input type="hidden" id="r_batch_no" value="${info.batchNo}">
									<input type="hidden" id="r_ph" value="${info.ph}">
									<input type="hidden" id="r_conductivity" value="${info.conductivity}">
									<input type="hidden" id="r_tds" value="${info.tds}">
									<input type="hidden" id="r_do_val" value="${info.doVal}">
...
<div class="modal fade" id="multiParameterInfoModal" style="overflow: auto;" tabindex="-1" role="dialog" data-backdrop="static" data-keyboard="false">
         <div class="modal-dialog modal-lg" role="document">
             ...
             <div class="modal-header bg-cyan" >
                 	...
                 	<td rowspan="3" class="text-center font-bold" style="font-size: 16px !important">Form: Multi ParaMeter Logbook Info </td>
                 	<td style="width:90px">Document No.</td>
                 	<td colspan="2">LB-DIL-CM-032</td>
                 	</tr>
                 	...
                 	<div class="modal-body">
                 		...
                 		<div class="row">
	                 		<div class="col-md-2 align-right top">
                            	<label for="item_id">Name Of The Item :</label>
                            </div>
                            <div class="col-md-4">
                           	<div class="form-group">
                                <select  id="item_id" name="itemId" class="js-example-theme-single form-control" style="width: 100%;" required="required">
                            ...
                            <div class="col-md-2 align-right top">
                            	<label for="operate_date">Date :</label>
                            </div>
                            <div class="col-md-4">
                            <div class="form-group">
                                <input type="text" id="operate_date" name="operateDate" value="" class="form-control" style="width: 30%;" autocomplete="off" required="required">
                           	</div>
                            </div>
                 		</div>
                 		<div class="row">
                            ...
                            <div class="col-md-4">
                            <div class="form-group">
                                <input type="text" id="ph" name="ph" value="" class="form-control" style="width: 30%;" autocomplete="off" required="required">
                           	</div>
                            </div>
                 		</div>
                 		<div class="row">
                 		...
                 		<div class="row">
	                 		<div class="col-md-2 align-right top">
                            	<label for="do_val">DO :</label>
                            </div>
                            <div class="col-md-4">
                            <div class="form-group">
                                <input type="text" id="do_val" name="doVal" value="" class="form-control" style="width: 100%;" autocomplete="off" required="required">
                           	</div>
                            </div>
                 		...
                 		<div class="row">
                            ...
                            <div class="col-md-4">
                           	<div class="form-group">
                                <select  id="clean_by" name="cleanBy" class="js-example-theme-single form-control" style="width: 100%;">
			                        <option value="">-Select Employee-</option>
			                        <c:forEach var="info" items="${employeeInfos}">
			                           	<option value="${info.id }">${info.empName}</option>
			                        </c:forEach>
...
function add(el) {
	...
	$('#operate_date').datepicker('setDate', new Date());
	$("#batch_no").val("");
	$("#ph").val("");
	$("#conductivity").val("");
	$("#tds").val("");
	...
	$("#clean_by").val("").trigger('change.select2');
...
function edit(el) {
	...
	var itemId 		 = $(el).closest("tr").find("#r_item_id").val();
	var operateDate  = $(el).closest("tr").find("#r_operate_date").val();
	var batchNo 	 = $(el).closest("tr").find("#r_batch_no").val();
	var ph 			 = $(el).closest("tr").find("#r_ph").val();
	var conductivity = $(el).closest("tr").find("#r_conductivity").val();
	var tds 		 = $(el).closest("tr").find("#r_tds").val();
	var doVal 		 = $(el).closest("tr").find("#r_do_val").val();
	...
	$("#id").val(Id);
	$("#item_id").val(itemId).trigger('change.select2');
	$('#operate_date').datepicker('setDate', convertMmDate(operateDate));
...
$("#multiParameterInfoForm").submit(function(event){
    ...
    if($(".alert-code").hasClass('hidden')){
    	$.ajax({	
	    	url : "${pageContext.request.contextPath}/logbook_chemi/saveMultiParaMeterInfos",
	        type: 'POST',
	        data: formData,
	        async: false,
...
function printReport(el) {
	var ajaxURL = "${pageContext.request.contextPath}/logbook_chemi/multi-parameter-log-print";
