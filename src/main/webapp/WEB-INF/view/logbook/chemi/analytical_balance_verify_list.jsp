...
<div class="container-fluid">
	<div class="block-header">
	   <span style="text-shadow: 2px 2px 2px #aaa;">DISSOLUTION INFO</span>
		
	</div>
		   <form method="post" id="analyticalVerifyForm" onkeypress="if (event.keyCode == 13) { return false; }">
					...
					<table class="table table-bordered table-striped table-hover js-basic-example dataTable" id="verifyTable">
						<thead>
							<tr>
								...
								<th class="align-center" style="width: 110px;">DONE BY</th>
								<th class="align-center" style="width: 180px;">CLEAN BY</th>
						...
						<tbody>
							...
							<tr>
								...
								<td class="align-center"><c:choose><c:when test="${(info.qcRefName !=null) && (not empty info.qcRefName)}">
									${info.qcRefName}
							    	</c:when>    
								    <c:otherwise>
									 N/A
...
$("#analyticalVerifyForm").submit(function(event){
	event.preventDefault();				
    var formData = $("#analyticalVerifyForm").serialize();
    ...
    if($(".alert-code").hasClass('hidden')){
    	$.ajax({	
	    	url : "${pageContext.request.contextPath}/logbook_chemi/verifyAnalyticalBalance",
	        type: 'POST',
	        data: formData,
	        async: false,
