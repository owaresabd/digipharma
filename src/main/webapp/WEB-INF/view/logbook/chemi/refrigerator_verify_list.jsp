...
<div class="container-fluid">
	<div class="block-header">
	   <span style="text-shadow: 2px 2px 2px #aaa;">REFRIGERATOR VERIFY INFO</span>
		   ...
		   <form method="post" id="waterPurificationUpdateForm" onkeypress="if (event.keyCode == 13) { return false; }">
					...
					<table class="table table-bordered table-striped table-hover js-basic-example dataTable" id="verifyTable">
						<thead>
							<tr>
								...
								<th class="align-center" style="width: 110px;">NAME OF ITEM</th>
								<th class="align-center" style="width: 110px;">EQUIPMENT</th>
								<th class="align-center" style="width: 110px;">AR No.</th>
								<th class="align-center" style="width: 110px;">SAMPLE TYPE</th>
								<th class="align-center" style="width: 50px;">QUANTITY</th>
								<th class="align-center" style="width: 50px;">DISCARDED QUANTITY</th>
								<th class="align-center" style="width: 100px;">DATE (In Chamber)</th>
								<th class="align-center" style="width: 100px;">DATE (Out Chamber)</th>
						...
						<tbody>
							...
							<tr>
								...
								<td class="align-center">${info.qty} ${info.unitName}</td>
								<td class="align-center">${info.discardQty} ${info.disUnitName}</td>
								<td class="align-center">
									<fmt:formatDate pattern="dd-MMM-yyyy" value="${info.inDate}" var="dateVal" />
	                              	<c:out value="${dateVal}"/>
								</td>
								<td class="align-center">
									<fmt:formatDate pattern="dd-MMM-yyyy" value="${info.outDate}" var="dateVal" />
	                              	<c:out value="${dateVal}"/>
								</td>
...
$("#waterPurificationUpdateForm").submit(function(event){
	event.preventDefault();				
    var formData = $("#waterPurificationUpdateForm").serialize();
    ...
    if($(".alert-code").hasClass('hidden')){
    	$.ajax({	
	    	url : "${pageContext.request.contextPath}/logbook_chemi/verifyRefrigerator",
	        type: 'POST',
	        data: formData,
	        async: false,
