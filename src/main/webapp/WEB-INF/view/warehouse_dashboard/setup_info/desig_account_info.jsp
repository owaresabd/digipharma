...
<!------------------------ Start: Create Product Color Types Modal -------------------->
 ...
 <div class="modal-header bg-blue-grey">
	<button type="button" class="mod-cl close load" data-dismiss="modal" aria-hidden="true">&times;</button>
   	<h4 class="modal-title align-center" id="defaultModalLabel">ACCOUNTABILITY INFO</h4>
</div>
                <form method="post" id="accountInfoForm"  onkeypress="if (event.keyCode == 13) { return false; }">
                 	<div class="modal-body">
                        ...
                        <div class="col-md-12">
                        	<div class="form-group">
								...
								<thead>
									<tr>
										<th class="align-center">AC/ NO</th>
								...
								<c:choose>
								    <c:when test="${accountInfoList.size() > 0}">
										<c:forEach var="info" items="${accountInfoList}" varStatus="counter">
										<tr>
											<td class="align-center">
												<span class="sn">${counter.count}</span>
											</td>
											<td class="align-center">
												<input style="width: 100%;" type="text" id="account_desc" name='accountDesc[]' value="${info.accountDesc}" required="required"/>
											</td>
											<td class="align-center">
								    ...
								    <c:otherwise>
								        <tr>
											...
											<td class="align-center">
												<input style="width: 100%;" type="text" id="account_desc" name='accountDesc[]' required="required"/>
											</td>
											<td class="align-center">
												<a class="btn-edit btn btn-xs" onclick="addRow(this)"><i class="glyphicon glyphicon-plus"></i></a>
												<a class="btn btn-xs waves-effect" onclick="remove(this)"><i class="material-icons">delete_forever</i></a>
...
var aTable = $('#accountTable').DataTable({
	"aaSorting" : [],
...
$('#accountTable_length, #accountTable_info, #accountTable_paginate, #accountTable_filter').hide();

function addRow(el){
	
	var accountNo = $('#accountTable >tbody >tr').length+1;
	var td1="<span class='sn'>"+accountNo+"</span>";
	var td2="<input style='width: 100%;' type='text' id='r_account_desc' name='accountDesc[]' required='required'/>";
	var html = '' 
			+ '<a class="btn-edit btn btn-xs" onclick="addRow(this)"><i class="glyphicon glyphicon-plus"></i></a>'
			+ '<a class="btn btn-xs waves-effect" onclick="remove(this)"><i class="material-icons">delete_forever</i></a>';
...
$("#accountInfoForm").submit(function(event){
    ...
    var formData = $("#accountInfoForm").serialize();
    
    if($(".alert-code").hasClass('hidden')){
    	$.ajax({	
	    	url : "${pageContext.request.contextPath}/designation/save-account-infos",
	        type: 'POST',
	        data: formData,
	        async: false,
