...
<div class="container-fluid">
	<div class="block-header">
	   <span style="text-shadow: 2px 2px 2px #aaa;">RETENTION SAMPLE REQUEST INFO</span>
					...
					<table class="table table-bordered table-striped table-hover js-basic-example dataTable" id="typeList">
						<thead>
							<tr>
								...
								<th class="align-left" style="width: 200px;">NAME OF MATERIALS</th>
								<th class="align-left" style="width: 150px;">DATE &amp; TIME</th>
								<th class="align-center">STATUS</th>
								<th class="align-center" style="width: 120px;">ACTION</th>
							</tr>
						</thead>
						<tbody>
							<c:forEach var="info" items="${retentionList}" varStatus="counter">
							<tr>
								<td class="field_type_code align-center">${counter.count}</td>
								<td class="field_type_code">${info.udSampleNo}</td>
								<td class="field_type_code">${info.materialName}</td>
								<td class="field_type_name">${info.reqDate}</td>
								<td class="field_type_status align-center" width="100px">
									    ...
									    <c:when test="${info.status =='R'}">
											<span class="badge bg-green">Sample Received</span>
									    </c:when>     
...
<div class="modal fade" id="retInfoModal" style="overflow: auto !important;"  role="dialog" data-backdrop="static" data-keyboard="false"  >
...
$(".load").on('click', function(e){
	var link =  "${pageContext.request.contextPath}/retention/maintain";
...
function view(el) {
	...
	$.ajax({
		async : false,
		url : "${pageContext.request.contextPath}/retention/getRetentionInfo/" + id ,
		success : function(data) {
			$('#retInfoModal').modal('show').find('.modal-content').html(data);
...
function add(el) {
	...
	$.ajax({
		async : false,
		url : "${pageContext.request.contextPath}/retention/sampleRcvForm/"+id,
		success : function(data) {
			$('#retInfoModal').modal('show').find('.modal-content').html(data);
			$(".modal-backdrop.fade.in").off("click");
			$(".modal").off("keydown");
