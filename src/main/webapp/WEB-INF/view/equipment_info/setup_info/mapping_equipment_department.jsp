...
<div class="container-fluid">
	<div class="block-header">
	    <span style="text-shadow: 2px 2px 2px #aaa;">EQUIPMENT MAPPING TO DEPARTMENT</span>
				...
				<div class="body">
					<div class="row">
						<div class="col-md-6">
							...
							<select id="department_list" class="js-example-theme-single form-control">
	                        		<option value="none">---SELECT DEPARTMENT---</option>
	                        		<option value="C">CHEMI</option>
...
$(".js-example-theme-single").select2({
	...
	$("#department_list").on('change', function() {
		var departmentId = $("#department_list option:selected").val();
		...
		if(departmentId == 'none'){
			...
			loadAssignedEquipment(departmentId);
			loadUnassignedEquipment();
		}
	});
	
	function loadAssignedEquipment(department_id) {
		$.ajax({
			type : "GET",
			url : "${pageContext.request.contextPath}/equipment/assignedEquipmentList/" + department_id ,
			dataType : 'json',
	...
	function loadUnassignedEquipment() {
		$.ajax({
			type : "GET",
			url : "${pageContext.request.contextPath}/equipment/unassignedEquipmentList",
			dataType : 'json',
			success : function(data) {
	...
	function addLogbookEquipment(equipmentId){
		...
		$.ajax({
			type : "POST",
			url : "${pageContext.request.contextPath}/equipment/save-equipment-mapping/" + equipmentId + "/" + departmentId,
			success : function() {
	...
	function removeLogbookEquipment(equipmentId){
		...
		$.ajax({
			type : "POST",
			url : "${pageContext.request.contextPath}/equipment/remove-equipment-mapping/" + equipmentId + "/" + departmentId,
			success : function() {
				console.log("save success");
			},
			error: function(){
