...
<div class="container-fluid">
	<div class="block-header">
	   <span style="text-shadow: 2px 2px 2px #aaa;">WORK INSTRUCTION REVISION INFORMATION</span>
			...
			<div class="card">
				<div class="body table-responsive table-bordered">
					...
					<table class="table table-bordered table-striped table-hover js-basic-example dataTable" id="insTable">
						...
						<tbody>
							<c:forEach var="info" items="${instructionRefList}" varStatus="counter">
							<tr>
								<td class="align-center">${counter.count}</td>
								<td class="field_type_remarks">${info.workInsUdId}</td>
								<td class="field_instruction_name">${info.insName}</td>
								...
								<td class="align-center">
									...
									<input type="hidden" id="r_instruction_name" value="${info.workInsId}">
									<input type="hidden" id="r_ins_ud_id" value="${info.workInsUdId}">
									...
									<c:if test="${not empty info.workInsDocName}">
									<a id="down_link" class="btn-edit btn btn-xs" href="${pageContext.request.contextPath}/image/work_instruction_doc/${info.workInsDocName}" target="_blank"><i class="fa fa-download"></i></a>
									</c:if>
								</td>
							</tr>
							</c:forEach>
...
<div class="modal fade" id="workInstructionModal"  role="dialog" data-backdrop="static" data-keyboard="false">
         <div class="modal-dialog" role="document">
             ...
             <div class="modal-header bg-cyan" >
                 	...
                 	<td rowspan="3" class="text-center font-bold" style="font-size: 16px !important">Form: Work Instruction Revision Info </td>
                 	<td>Document No.</td>
                 ...
                 <form method="post" id="workInstructionRefForm" modelAttribute="instructionRefInfo" enctype="multipart/form-data">
                 	<div class="modal-body">
                 		...
                 		<div class="row">
                            ...
                            <div class="col-md-8">
	                            <div class="form-group">
	                            	<select id="instruction_no" name="workInsId" class="js-example-theme-single form-control" required="required" style="width: 100%;">
				                        	<option></option>
				                        <c:forEach var="info" items="${instructionList}">
				                           	<option value="${info.id }">${info.instructionName}</option>
				                        </c:forEach>
				                        </select>
	                            </div>
                            </div>
                 		...
                 		<div class="row">
                            ...
                            <div class="col-md-8">
	                            <div class="form-group">
	                            	<input type="text" id="ud_instruction_no" name="workInsUdId" class="form-control" placeholder="Instruction No"  autocomplete="off" readonly="readonly">
	                            </div>
                            </div>
                 		</div>
                 		<div class="row">
                            ...
                            <div class="col-md-8 m-t-15">
                            	<div class="form-group">
                                	<input type="text" id="revision_no" name="revisionNo" class="form-control" placeholder="Revision No"  autocomplete="off" >
	                            </div>
                            </div>
                 		</div>
                 		<div class="row">
                            ...
                            <div class="col-md-8 m-t-15">
                            	<div class="form-group">
                                	<input type="date" id="effective_date" name="effectiveDate" class="form-control" placeholder="Effective Date"  autocomplete="off" >
	                            </div>
                            </div>
                 		</div>
                 		<div class="row">
...
function edit(el) { 
	...
	var instEffictiveDate 	= $(el).closest("tr").find("#r_effective_date").val();
	var instRevisionNo 	= $(el).closest("tr").find("#r_revision_no").val();
	...
	$("#id").val(Id);
	$("#instruction_no").val(instName).trigger('change.select2');
	$("#ud_instruction_no").val(instUdId);
	$("#effective_date").val(instEffictiveDate);
	$("#revision_no").val(instRevisionNo);
	$("#remarks").val(remarks);
...
$("#workInstructionRefForm").submit(function(event){
	event.preventDefault();				
	  var formData = new FormData($("#workInstructionRefForm")[0]);
    swal({
        title: "Are you sure?",
    ...
    if($(".alert-code").hasClass('hidden')){
    	$.ajax({	
	    	url : "${pageContext.request.contextPath}/workInstruction/save-workInstructionRef-info",
	        type: 'POST',
	        data: formData,
