...
<div class="container-fluid">
	<div class="block-header">
	   <span style="text-shadow: 2px 2px 2px #aaa;">QC REQUEST LIST</span>
					...
					<table class="table table-bordered table-striped table-hover js-basic-example dataTable" id="typeList">
						<thead>
							<tr>
								...
								<th class="align-left" style="width: 150px;">REQUEST DATE &amp; TIME</th>
								<th class="align-center">STATUS</th>
								<th class="align-center" style="width: 120px;">ACTION</th>
							</tr>
						</thead>
						<tbody>
							<c:forEach var="info" items="${qcList}" varStatus="counter">
							<tr>
								...
								<td class="align-center">
									...
									<c:choose>
									    <c:when test="${info.typeCode =='PT' || info.status =='AV'}">
											 <a class="btn-edit btn btn-xs" onclick="viewPTSample(this)"><i class="material-icons">visibility</i></a>
									    </c:when>
									    <c:otherwise>
...
$(".load").on('click', function(e){
	var link =  "${pageContext.request.contextPath}/qc/maintain";
...
function view(el) {
	...
	$.ajax({
		async : false,
		url : "${pageContext.request.contextPath}/qc/getQcInfo/" + id ,
...
function add(el) {
	...
	$.ajax({
		async : false,
		url : "${pageContext.request.contextPath}/qc/sampleRcvForm/"+id,
		success : function(data) {
			$('#qcInfoModal').modal('show').find('.modal-content').html(data);
			$(".modal-backdrop.fade.in").off("click");
			$(".modal").off("keydown");
