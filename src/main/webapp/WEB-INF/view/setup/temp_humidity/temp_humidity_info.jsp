...
<div class="container-fluid">
	<div class="block-header">
	   <span style="text-shadow: 2px 2px 2px #aaa;">TEMPERATURE &amp; HUMIDITY INFO</span>
					...
					<table class="table table-bordered table-striped table-hover js-basic-example dataTable" id="typeList">
						<thead>
							<tr>
								...
								<th class="align-center" style="width: 110px;">CORRECTION VALUE FOR TEMPERATURE</th>
								<th class="align-center" style="width: 110px;">CORRECTION VALUE FOR HUMIDITY</th>
						...
						<tbody>
							...
							<tr>
								...
								<td class="align-center">${info.departmentByName}</td>
								<td class="align-center">${info.locationByName}</td>
								<td class="align-center">${info.correctionValFrTemp}</td>
								<td class="align-center">${info.correctionValFrHumidity}</td>
								...
								<td class="align-center">
									...
									<input type="hidden" id="r_equipment_nm" value="${info.equipmentByName}">
									<input type="hidden" id="r_location_id" value="${info.locationId}">
									<input type="hidden" id="r_department_id" value="${info.departmentId}">
									<input type="hidden" id="r_value_temp" value="${info.correctionValFrTemp}">
									<input type="hidden" id="r_value_humidity" value="${info.correctionValFrHumidity}">
         ...
         <div class="modal-dialog" role="document">
             <div class="modal-content">
                  <div class="modal-header bg-blue-grey">
                    ...
                    <h4 class="modal-title align-center" id="defaultModalLabel">TEMPERATURE &amp; HUMIDITY SETUP INFO</h4>
                 </div>
             
                 <form method="post" id="tempHumidityInfoForm">
                 	<div class="modal-body">
                 		...
                 		<div class="row">
                            ...
                            <div class="col-md-4">
                 			...
                 			<div class="col-md-2 align-right top">
                            	<label for="equipment_id">DEPARTMENT</label>
                            </div>
                            <div class="col-md-4">
	                            <div class="form-group">
	                            	<select id="department_id" name="departmentId" class="js-example-theme-single form-control" style="width: 100%;" required="required">
				                        <option value="">-Select Department-</option>
				                        <c:forEach var="info" items="${deptInfos}">
				                           	<option value="${info.id}">${info.deptName}</option>
				                        </c:forEach>
				                    </select>
	                            </div>
                            </div>
                 		...
                 		<div class="row">
	                 		<div class="col-md-2 align-right top">
                            	<label for="qc_reference">LOCATION</label>
                            </div>
                            <div class="col-md-4">
                           	<div class="form-group">
                 		...
                 		<div class="row">
	                 		<div class="col-md-6 align-right top">
                            	<label for="document_no">CORRECTION VALUE FOR TEMPERATURE</label>
                            </div>
                            <div class="col-md-6">
                            <div class="form-group">
                                <input type="text" id="correction_val_fr_temp" name="correctionValFrTemp" value="" class="form-control" style="width: 100%;" autocomplete="off" required="required">
                           	</div>
                            </div>
                 		</div>
                 		...
                 		<div class="row">
	                 		<div class="col-md-6 align-right top">
                            	<label for="revision_no">CORRECTION VALUE FOR HUMIDITY</label>
                            </div>
                            <div class="col-md-6">
                            <div class="form-group"> 
                                <input type="text" id="correction_val_fr_humidity" name="correctionValFrHumidity" value="" class="form-control" style="width: 100%;" autocomplete="off" required="required">
                           	</div>
                            </div>
                 		</div>
...
$(".load").on('click', function(e){
	var link =  "${pageContext.request.contextPath}/tempHumiditySetup/maintain";
...
function add(el) {
	...
	$("#correction_val_fr_temp").val("");
	$("#correction_val_fr_humidity").val("");
...
function edit(el) {
	...
	var equName = $(el).closest("tr").find("#r_equipment_nm").val();
	var lcId 	= $(el).closest("tr").find("#r_location_id").val();
	var dpetId 	= $(el).closest("tr").find("#r_department_id").val();
	var tempVal 	= $(el).closest("tr").find("#r_value_temp").val();
	var humidityVal 	= $(el).closest("tr").find("#r_value_humidity").val();
	var status 		= $(el).closest("tr").find("#r_status").val();
		
	$("#id").val(Id);
	$('#equipment_id').append($("<option></option>").attr("value", equId).text(equName)); 
	...
	$("#correction_val_fr_temp").val(tempVal);
	$("#correction_val_fr_humidity").val(humidityVal);
	if(status == 'Y'){
		$('#activity_status').prop('checked', true);
		$('#status').val('Y');
		$('.check-status').text('Active ?');
...
$("#tempHumidityInfoForm").submit(function(event){
	event.preventDefault();				
    var formData = $("#tempHumidityInfoForm").serialize();
    swal({
        title: "Are you sure?",
    ...
    if($(".alert-code").hasClass('hidden')){
    	$.ajax({	
	    	url : "${pageContext.request.contextPath}/tempHumiditySetup/saveTempHumiditySetupInfos",
	        type: 'POST',
	        data: formData,
	        async: false,
