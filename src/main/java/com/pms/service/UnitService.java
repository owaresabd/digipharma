...
import com.pms.repository.UnitRepository;
...
public class UnitService {
	...
	public List<UnitInfo> getAll(String status) {
		User user = userService.getCurrentUser();
		if (status != null) {
			return unitRepository.findAll().stream()
					.filter(info -> info.getStatus().equals(status) && info.getCompanyId().equals(user.getCompanyId()))
					.sorted(Comparator.comparing(c -> c.getUnitName(), String.CASE_INSENSITIVE_ORDER))
	...
	public void saveOrUpdate(UnitInfo info) {
		User user = userService.getCurrentUser();
		if (info.getId() == null ) {
