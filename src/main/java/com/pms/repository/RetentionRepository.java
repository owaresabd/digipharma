...
public class RetentionRepository {
	...
	public List<CommonInfo> findAll() {
		...
		return jdbcTemplate.query(
				" select e.id,a.ud_css_no, a.wh_request_id,a.req_by,coalesce(to_char( a.req_at, 'DD-Mon-YYYY HH12:MI:SS AM'), '') as req_at,a.req_status css_status,"
				...
				+ " b.rcv_qty, b.manufacture_name,  b.country_id,fnc_country_nm(b.country_id) country_nm, b.sample_details, b.req_status wh_status, c.is_chemical,c.is_microbiological ,c.total_amt, c.unit_id,fnc_unit_nm(c.unit_id) mat_unit_name,"
				...
				+ " e.sample_id, e.sample_desc, e.uncertinity, coalesce(to_char( e.qc_req_dt, 'DD-Mon-YYYY HH12:MI:SS AM'), '') as ret_req_dt, e.status ret_status, "
				...
				+ " from lims_css_request_infos a, lims_wh_request_infos b, lims_material_infos c, lims_sampling_infos d, lims_retention_req_infos e "
				+ "  where  a.wh_request_id=b.id and b.material_id=c.id and a.id=d.css_request_id and d.id=e.sample_id and e.company_id = ? ORDER BY e.qc_req_dt DESC",
				new Object[] { user.getCompanyId() }, new RetentionInfoRowMapper());
	}

	
	public CommonInfo getRetentionInfo(UUID id){
		jdbcTemplate = new JdbcTemplate(datasource);
		return jdbcTemplate.queryForObject(" select e.id,a.ud_css_no, a.wh_request_id,a.req_by,coalesce(to_char( a.req_at, 'DD-Mon-YYYY HH12:MI:SS AM'), '') as req_at,a.req_status css_status,"
				+ " b.req_type,fnc_req_type_nm(b.req_type) req_type_name, b.effective_date, b.revision_no, b.form_no, b.invoice_no, b.chalan_no, b.lc_no,"
				+ "  b.from_dept_no, fnc_dept_nm(b.from_dept_no) from_dept_nm, b.to_dept_no,fnc_dept_nm(b.to_dept_no) to_dept_nm,"
				+ " b.gr_no, b.material_id, fnc_material_nm(b.material_id) material_name, b.material_code, b.material_type_id, fnc_material_type_nm(b.material_type_id) material_type_nm,"
				+ " b.rcv_qty, b.manufacture_name,  b.country_id,fnc_country_nm(b.country_id) country_nm, b.sample_details, b.req_status wh_status, c.is_chemical,c.is_microbiological ,c.total_amt, c.unit_id,fnc_unit_nm(c.unit_id) mat_unit_name,"
				...
				+ " and e.id = ?",
				 new Object[] { id}, new RetentionInfoRowMapper());
	}
	public List<CommonInfo> receivedList() {
		...
		return jdbcTemplate.query(
				...
				+ " sampling_date, sampling_name, status, remarks, ud_qc_no, sample_id, sample_desc, uncertinity, qc_req_dt, "
				+ " ret_status, company_id, ret_req_id, arn_no, sample_desc_rcv, ret_qty, ret_unit_id, ret_unit_name, ret_equ_id, ret_equ_name,"
				...
				+ "	FROM view_ret_sample_rcv_infos where ret_status='R' and  company_id = ? ORDER BY receive_at DESC",
	...
	public CommonInfo getSampleReceiveInfo(UUID id){
		...
		return jdbcTemplate.queryForObject(
				...
				+ " (CAST((CASE WHEN chemical_sample_amt ='' THEN '0' ELSE chemical_sample_amt END)  as float) + CAST((CASE WHEN micro_sample_amt ='' THEN '0' ELSE  micro_sample_amt END)  as float)) total_sample_amt, "
				+ " (CAST((CASE WHEN chemical_retention_amt ='' THEN '0' ELSE chemical_retention_amt END)  as float) + CAST((CASE WHEN micro_retention_amt ='' THEN '0' ELSE  micro_retention_amt END)  as float)) total_retention_amt "
				+ " FROM view_ret_sample_rcv_infos where id = ?",
	...
	public void saveSampleRcvInfo(RetentionSampleRecvInfo info) {
		jdbcTemplate = new JdbcTemplate(datasource);
		User user = userService.getCurrentUser();
		Timestamp time = new Timestamp(System.currentTimeMillis());
		
			jdbcTemplate.update(
					"INSERT INTO lims_ret_sample_rcv_infos (ret_req_id, sample_desc, ret_qty, ret_unit_id, ret_equ_id, ret_room_id, ret_rack_id, "
					+ "status, sample_id, company_id, created_by, created_at) "
					+ "VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",info.getRetReqId(), info.getSampleDesc(), info.getRetQty(),info.getRetUnitId(), info.getRetEquId(), 
					info.getRetRoomId(), info.getRetRackId(), "P", info.getSampleId(), user.getCompanyId(), user.getId(), time);
			//jdbcTemplate.update("update lims_ret_sample_rcv_infos set arn_no ='"+arntNoGen(info.getSampleId())+"' where sample_id = ?", info.getSampleId());
			jdbcTemplate.update("UPDATE lims_retention_req_infos SET status ='R' where id = ?", info.getRetReqId());
			
			
		
	}
	public   String  arntNoGen(UUID id) {
		jdbcTemplate = new JdbcTemplate(datasource);
		...
		CommonInfo info = jdbcTemplate.queryForObject("SELECT a.type_no,c.company_code,b.is_chemical,b.is_microbiological,b.sample_id , " + 
				"(SELECT COALESCE(CAST(MAX(RIGHT(arn_no,4)) as integer),0)+1 from lims_ret_sample_rcv_infos)  max_id ,b.material_name " + 
		...
		if(info.getIsChemical().equals("Y") && info.getIsMicrobiological().equals("Y")) {
			udSampleId.append("ARN-").append("GN-") .append(info.getMaterialTypeName()) .append("-").append(info.getCompanyCode()).append("-").append(year).append("-").append(StringUtils.leftPad(info.getMaxId(), 4, "0"));
		}else if(info.getIsChemical().equals("Y") && info.getIsMicrobiological().equals("N")) {
			
			udSampleId.append("ARN-").append("CM-") .append(info.getMaterialTypeName()) .append("-").append(info.getCompanyCode()).append("-").append(year).append("-").append(StringUtils.leftPad(info.getMaxId(), 4, "0"));
			
		}else if(info.getIsChemical().equals("N") && info.getIsMicrobiological().equals("Y")) {
			
			udSampleId.append("ARN-").append("MC-") .append(info.getMaterialTypeName()) .append("-").append(info.getCompanyCode()).append("-").append(year).append("-").append(StringUtils.leftPad(info.getMaxId(), 4, "0"));
	...
	class RetentionInfoRowMapper implements RowMapper<CommonInfo> {
		...
		public CommonInfo mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			CommonInfo info = new CommonInfo();
			  ...
			  info.setUdCssNo(rs.getString("ud_css_no"));
			  info.setWhRequestId((UUID)rs.getObject("wh_request_id"));
			  info.setReqDate(rs.getString("ret_req_dt"));
			  info.setStatus(rs.getString("ret_status"));
			  info.setReqType((UUID)rs.getObject("req_type"));
			  info.setReqTypeName(rs.getString("req_type_name"));
			  info.setEffectivDate(rs.getDate("effective_date"));
			  info.setRevisionNo(rs.getString("revision_no"));
