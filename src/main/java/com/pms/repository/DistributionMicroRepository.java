...
@Repository
@Transactional
public class DistributionMicroRepository {
	@Autowired
	...
	public List<CommonInfo> findAll() {
		...
		return jdbcTemplate.query(
				...
				+ " country_id, country_nm, sample_details, wh_status, is_chemical, is_microbiological,chemical_sample_amt, micro_sample_amt, "
				+ " total_amt,mat_unit_name, ud_sample_no, client_name,"
				...
				+ " ret_room_id, ret_room_name, ret_rack_id, ret_rack_name, micro_dist_status, receive_at, micro_dist_at, "
				...
				+ "	FROM view_sample_rcv_infos where micro_dist_status='P' and is_microbiological='Y' and   company_id = ? ORDER BY receive_at DESC",
	...
	public List<CommonInfo> findDistributedList() {
		...
		return jdbcTemplate.query(
				...
				+ " (CAST((CASE WHEN chemical_sample_amt ='' THEN '0' ELSE chemical_sample_amt END)  as float) + CAST((CASE WHEN micro_sample_amt ='' THEN '0' ELSE  micro_sample_amt END)  as float)) total_sample_amt, "
				+ " (CAST((CASE WHEN chemical_retention_amt ='' THEN '0' ELSE chemical_retention_amt END)  as float) + CAST((CASE WHEN micro_retention_amt ='' THEN '0' ELSE  micro_retention_amt END)  as float)) total_retention_amt "
				+ "	FROM view_sample_rcv_infos where micro_dist_status='D' and is_microbiological='Y' and  company_id = ? ORDER BY receive_at DESC",
				new Object[] { user.getCompanyId() }, new ReceiveInfoRowMapper());
	}
	...
	public CommonInfo getSampleReceiveInfo(UUID id){
		...
		return jdbcTemplate.queryForObject(
				 ...
				 + "ret_room_name, ret_rack_id, ret_rack_name, micro_dist_status, receive_at, micro_dist_at, "
				 + " (CAST((CASE WHEN chemical_sample_amt ='' THEN '0' ELSE chemical_sample_amt END)  as float) + CAST((CASE WHEN micro_sample_amt ='' THEN '0' ELSE  micro_sample_amt END)  as float)) total_sample_amt, "
				 + " (CAST((CASE WHEN chemical_retention_amt ='' THEN '0' ELSE chemical_retention_amt END)  as float) + CAST((CASE WHEN micro_retention_amt ='' THEN '0' ELSE  micro_retention_amt END)  as float)) total_retention_amt "
	...
	public void saveDistributionInfo(DistributionInfo info) {
		...
		jdbcTemplate.update(
					...
					+ "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
					 ...
					 info.getDistAnalystId(),"P","M", info.getSampleId(),info.getDistLod(), user.getCompanyId(), user.getId(), time);
		//jdbcTemplate.update("UPDATE lims_qc_sample_rcv_infos SET micro_dist_status ='D' where id = ?", info.getSampleRcvId());
		
	}
	
	public void updateMicroDistributionInfo(UUID sampleRcvId) {
		jdbcTemplate = new JdbcTemplate(datasource);
		User user = userService.getCurrentUser();
		Timestamp time = new Timestamp(System.currentTimeMillis());
		
		jdbcTemplate.update("UPDATE lims_qc_sample_rcv_infos SET micro_dist_status ='D', micro_dist_by =?, micro_dist_at =? where id = ?", user.getId(), time, sampleRcvId);
	...
	class ReceiveInfoRowMapper implements RowMapper<CommonInfo> {
		...
		public CommonInfo mapRow(ResultSet rs, int rowNum) throws SQLException {			
			CommonInfo info = new CommonInfo();
			  ...
			  info.setStatus(rs.getString("micro_dist_status"));
